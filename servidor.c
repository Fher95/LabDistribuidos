...
comprarhamburguesasistema_1_svc(nodo_hamburguesa_factura *argp, struct svc_req *rqstp)
{
	...
	if (hamburguesaRepetida((*argp).nombre)== FALSE){
		...
		printf("	Nombre Cabeza: %s \n", cabeza->nombre);
		printf("	Id Cliente Cabeza: %i \n", cabeza->idCliente);
		result=TRUE;
...
bool_t *
verificar_capcha_1_svc(char **argp, struct svc_req *rqstp)
{
	static bool_t  result;

	result = FALSE;
	if (strcmp(varCapcha,(*argp))==0){
		result = TRUE;
	}
...
char **
recibir_capcha_1_svc(void *argp, struct svc_req *rqstp)
{
	static char * result;

	int varNum;
	srand(time(NULL));
	varNum = 100 + rand()%(1000-100);
	sprintf(varCapcha,"%i",varNum);
	result = (char*)malloc(sizeof(char)*10);
	strcpy(result,varCapcha);
